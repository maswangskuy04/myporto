...
function Projects() {
  ...
  const toolIcons = {
    ...
    'Tailwind CSS': <Icon icon="devicon:tailwindcss" />,
    'Laravel': <Icon icon="devicon:laravel" />,
    ...
    {selectedProject && (
      <div className="fixed inset-0 bg-black bg-opacity-50 z-50 overflow-y-auto px-4 py-8" onClick={closeModal}>
        <div className="flex min-h-full items-center justify-center">
          <div className={`bg-white dark:bg-[#242423] rounded-lg w-full max-w-xl mx-auto relative transform transition-all duration-300 ${showModal ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} onClick={(e) => e.stopPropagation()}>
            <div className="max-h-[90vh] overflow-y-auto p-6">
              ...
              <p className="text-gray-700 dark:text-gray-400 mb-6">{selectedProject.description}</p>
              <div className="flex flex-wrap gap-2">
                {selectedProject.tools.map((tool, i) => (
                  <div key={i} className="flex items-center gap-2 bg-gray-100 dark:bg-black/50 px-3 py-1 rounded-full text-sm text-gray-700 dark:text-gray-200">
                    {toolIcons[tool]} {tool}
                  </div>
                ))}
              </div>
